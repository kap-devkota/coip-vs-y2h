...
```{r test2-cwn-l3}
getd2d3pairs <- function(G, gcommon, n_samples, get_d3 = TRUE) {
  t_samples <- n_samples * 10
  maxdeg <- ifelse(get_d3, 4, 3)
  prospective_samples <- data.frame(do.call(rbind, lapply(1:t_samples, function(i) {gcommon, 2)})))
  prospective_samples$distances <- apply(prospective_samples, 1, function(x) {
    return(igraph::distances(G, v=x[1], to=x[2])[1, 1])
  })
...
results$scor <- apply(results, 1, function(x) {
  no_samples <- x[[1]]
  samples <- getd2d3pairs(Gcs, gcommon, no_samples, FALSE) # get only distance 2 pairs
  
  dsd_coip <- apply(samples, 1, function(x) {return(CWMat_coip[x[[1]], x[[2]]])})
  dsd_y2h <- apply(samples, 1, function(x) {return(CWMat_y2h[x[[1]], x[[2]]])})
  spearman <- scorr(dsd_coip, dsd_y2h)
  return(spearman)
})

CWMat_coip[V(Gcs)$name[1], V(Gcs)$name[2]] 
