...
pprots <- get_uniprot(proteases$symbol, gcommon)
kprots <- get_uniprot(kinases$symbol, gcommon)
pkprots <- c(pprots, kprots)
get_conductance <- function(G, nodes) {
  vol <- sum(degree(G)[nodes])
}
dgc_sp <- dgc_s[which(dgc_s$V1 %in% pkprots | dgc_s$V2 %in% pkprots), ] 
dgc_sp <- dgc_sp[which(!(dgc_sp$V1 %in% pkprots & dgc_sp$V2 %in% pkprots)), ]

conductance_c <- length(dgc_sp) / sum(degree(Gc)[pkprots])
conductance_c

dgy_sp <- dgy_s[which(dgy_s$V1 %in% pkprots | dgy_s$V2 %in% pkprots), ]
dgy_sp <- dgy_sp[which(!(dgy_sp$V1 %in% pkprots & dgy_sp$V2 %in% pkprots)), ]

conductance_y <- length(dgy_sp) / sum(degree(Gy)[pkprots])
