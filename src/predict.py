...
def wmv(A, labels_f, weight_f=lambda x: x, default_label="????", confidence = True, k = -1, params = None):
    """
    Weighted majority vote algorithm for an undirected graph.

    Input:
      - An adjacency matrix for a graph. Should be non-negative
      ...
      - k - number of neighbors (if -1, select all neighbors)
    Output:
    ...
    for i in range(n):
        ...
        # Ensure that the diagonal component is zero and extract the neighbors associated with the largest similarity scores.
        # When k = -1 ( we are automatically igonring the diagonal entry (it being zero) as A should be non-negative
        A[i, i] = 0
        voters_ = np.argsort(-A[i])[:k]
        
        
        voters_ = filter(lambda j: A[i, j] != 0, voters_)  # We are filtering out the entries with value zero: 
        if filter_type == "degree":
            voters     = []
            count      = 0
            for v in voters_:
...
def mv(A, labels_f, confidence = True, k = -1, default_label="????"):
    """
    ...
    Output:
      ...
      is already known, the first label in the list is picked.
    """
    return wmv(A, labels_f, weight_f=lambda _: 1, default_label=default_label, confidence = confidence, k = k)
